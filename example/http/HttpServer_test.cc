...
*   File Name     : HttpServer_test.cc
*   Last Modified : 2019-11-21 17:03
*   Describe      :
*
*******************************************************/
...
void onRequest(const HttpRequest& req, HttpResponse* resp) {
    std::cout << "Headers " << req.methodToString() << " " << req.getPath() << std::endl;
    if(show) {
    ...
    if(req.getPath() == "/") {
        ...
        resp->addHeader("Server", "ideal");
        std::string now =ideal::Timestamp::now().toFormattedString();
...
int main(int argc, char* argv[]) {
    ...
    if(argc > 1) {
        show = false;
        ideal::Logger::setLogLevel(ideal::Logger::WARN);
        numThreads = atoi(argv[1]);
    }

    ideal::net::EventLoop loop;
    HttpServer server(&loop, ideal::net::InetAddress(8000), "HttpServer");
