...
*   File Name     : RtspConnection.cc
*   Last Modified : 2019-12-02 20:01
*   Describe      :
*
*******************************************************/
...
#include "ideal/media/RtspMessage.h"
#include "ideal/media/MediaSource.h"
#include "ideal/media/RtpConnection.h"
...
RtspConnection::RtspConnection(Rtsp* rtsp, const ideal::net::TcpConnectionPtr& conn) :
    _rtsp(rtsp),
    _tcpConn(conn),
...
void RtspConnection::handleRtspMessage(ideal::net::Buffer* buf) {
...
bool RtspConnection::handleRtspRequest(ideal::net::Buffer* buf) {
#ifdef RTSP_DEBUG
...
void RtspConnection::handleRtcp(ideal::net::Buffer* buf) {
...
void RtspConnection::handleRtspResponse(ideal::net::Buffer* buf) {
#ifdef RTSP_DEBUG
    std::string str(buf->peek(), buf->readableBytes());
    if(str.find("rtsp") != std::string::npos || str.find("RTSP") != std::string::npos) {
        std::cout << str << std::endl;
